...
export default function DataUsageTable({ refreshTrigger }) {
    const [dataUsage, setDataUsage] = useState([]);
    const dataTableRef = useRef(null); // Reference to the DataTable instance

    // Fetch data usage logs
    useEffect(() => {
        async function fetchDataUsage() {
            try {
                const response = await fetch("/api/fetchDataUsage");
                const result = await response.json();

                if (result.success) {
                    setDataUsage(result.data);
                } else {
                    console.error("Failed to fetch data usage:", result.error);
                }
            } catch (error) {
    ...
    useEffect(() => {
        const table = document.getElementById("dataUsageTable");
    ...
    return (
        <div className="max-w-6xl mx-auto mt-6">
            <h2 className="text-xl text-gray-700 font-bold mb-4">Data Usage</h2>
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table id="dataUsageTable" className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
